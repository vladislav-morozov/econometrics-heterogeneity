# The Within Estimator Under Heterogeneity
...
### First Focus: Workhorse Estimators under Heterogeneity

Can existing workhorse estimators for linear panel data models — the within (fixed effects) and dynamic IV estimators — meaningfully estimate $\E[\bbeta_i]$? Of course, those methods were developed in the context of the simpler random intercept model  @eq-random_intercept
$$
...
Regrettably, we will see that the "standard" estimators will generally fail in the presence of coefficient heterogeneity, both for static and the dynamic formulations of @eq-within-lecture-model. 
 
### This Section: Static Model Assumption and Fixed $T$ 

In this section, we begin by considering the static case and the associated workhorse estimator — the within (fixed effects) estimator. 
By "static", we mean that the model satisfies strict exogeneity in the form
...
## The Within Estimator with Heterogeneous Coefficients
...
<!-- To study the within estimator under heterogeneity, we strengthen strict exogeneity to hold in an augmented form that also conditions on the coefficient vector $\bbeta_i$:

$$
\E[\bu_i|\bX_i, \bbeta_i] = 0.
$$
  -->
...
Even though $\E[\bEta_i]=0$, the above condition does not necessarily hold if $\bbeta_i$ and $\bX_i$ are allowed to dependent.
 ...
 If condition [-@eq-average_orthogonality_condition] holds, the within estimator is consistent for $\E[\bbeta_i]$ in the heterogeneous coefficient model @eq-within-lecture-model. If it fails, the estimator is subject to *heterogeneity bias* [@Campello2019].
...
Under this condition 	$\E[\tilde{\bX}_i'\tilde{\bX}_i\bEta_i] =0$, and thus the within estimator is consistent for $\E[\bbeta_i]$. These conditions were proposed by @Wooldridge2003, @Wooldridge2005.
...
### Intuition for the Mean Independence Condition
...
### On The Usefulness of Panel Data


It is useful  to contrast condition [-@eq-mean_independence_condition] with the stronger condition 
$$
\E[\bEta_i|\bX_i]=0.
$$ 
