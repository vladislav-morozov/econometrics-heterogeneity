...
This section discusses the problem of unobserved heterogeneity, its kinds, and sets out some common themes for the course. 
...
Our objective is to estimate causal parameters: treatment effects (or their moments and distributions), policy impacts, or other structural quantities that describe how outcomes would change under interventions. Unlike predictive modeling, we focus on counterfactuals, rather than associational patterns.
...
A key challenge to our is the presence of $(A_i, U_{it})$ which are never observed but systematically influence both treatments and outcomes.
...
Throughout, we will focus on non-experimental (observational) data, where unobserved heterogeneity cannot be ignored. We will also allow non-binary and continuous treatments. We will emphasize identification strategies over asymptotic theory.
...
|--------------------------------|---------------------------------------------|------------------------------------|-----------------------------------------|
| **Linear Heterogeneous Models** | Linearity of $\phi$: $\phi(X_{it}, A_i, U_{it})$ $=$ $X_{it}'\beta(A_i)$ $+ U_{it}$ | Distribution and moments of individual treatment effects     |  Full distribution of treatment effects    |
