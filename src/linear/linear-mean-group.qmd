...
We can also express the corresponding realized outcome equations in unit-level matrix form by stacking observations across $t$ for each unit $i$:
$$
...
::: {.callout-note appearance="simple" icon=false} 
We implicitly assume the existence of any required moment involving $\left(\bX_i'\bX_i\right)^{-1}$. This assumption is not always innocent and imposes certain constraints on the distribution of covariates — requirements of sufficient variation. We do not discuss this point further, but see @Graham2012. 
:::
