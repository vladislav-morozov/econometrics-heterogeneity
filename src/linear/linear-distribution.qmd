...
$$
As in @sec-linear-variance, we impose

- Strict exogeneity
$$
\E[\bu_i|\bbeta_i, \bX_i] = 0.
$$
- Existence of the first two moments of $\bbeta_i, \bX_i$, and $\bu_i$.
...
To identify  $\varphi_{\bbeta_i|\bX_i}(\bs|\bX)$, we will proceed similarly to how we did with variance. The function of interest can be recovered @eq-linear-distribution-chf-estimators provided that we know $\varphi_{\bu_i|\bX_i}(\cdot|\bX)$. Accordingly, we will proceed in two steps:
...
\end{aligned}
$$
We conclude that the characteristic function of $\bu_i$ is identified. This completes the first step of our identification strategy. 

### Identifying the Distribution of Coefficients

For the second step — identification of $\varphi_{\bbeta_i|\bX_i}$ — we return to @eq-linear-distribution-chf-estimators. Here we have to make an additional assumption:
$$
...
Finally, the density or cumulative distribution functions of the coefficients may be recovered from by above expression using inversion formulae. For example, if the coefficients are continuously distribution, their conditional density satisfies
$$
