...
Having established identification for DTEs, we now turn to the question of QTEs. 

As in the previous section, we maintain the (conditional) unconfoundedness assumption ([-@eq-qte-unconf]).

$$
 \curl{Y_i^x}_x \independent X_i|W_i.
$$ 

As laid out in section [-@sec-qte-intro], we focus on the following parameters.
...
Yet again, we begin with the conditional quantiles of $Y^x_i$ given $W_i=w$.
...
In the next section, we move towards estimation and briefly discuss the essential aspects of quantile regression.