...
$$ {#eq-qte-unconf}
This setting is depicted on @fig-swig-obs. The setting of @eq-qte-unconf may arise both in observational settings and in experiments where randomization occurs within strata defined by $W_i$.
...
Note that the same result can be obtained by simply integrating $W_i$ out in the conditional CDF expression in @eq-qte-dte-cond-id. Since the CDF is an average, @wrn-qte-intro-cond does not apply to it. However, there is merit in studying the conditioning argument presented above, as it is useful both for the remaining conditional CDF and gives a path forward for unconditional quantiles (to which @wrn-qte-intro-cond does apply).
...
::: {.callout-note title="Counterfactual distributions"}

Integrals of the kind
$$
\int F_{Y|X, W}(y|x_1, w) F_{W|X}(dw|x_2),
$$ {#eq-dte-counterf-distr}
are sometimes called *counterfactual distributions* [@Machado2005CounterfactualDecompositionChanges; @Chernozhukov2013b].
...
Note that the counterfactual distribution ([-@eq-dte-counterf-distr]) is expressed in terms of distributions of realized variables. It only acquires a causal interpretations under suitable conditions (e.g. unconfoundedness, as we describe in this section).
