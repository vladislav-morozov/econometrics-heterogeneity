...
The argument is similar to that of marginal CDFs and quantiles, and again relies on the independence restriction ([-@eq-qte-rct]). For example, the identification argument for the CDF looks as follows:
$$
\begin{aligned}
...
\end{aligned}
$$
where $F_{Y|X, W}(y|x, w)$ is the CDF of the realized outcome $Y_i$ in the subpopulation of units with $X_i=x$ and $W_i=w$. As before, the group of units with $X_i=x$ identifies the distribution of potential outcomes under $x$. The argument for the quantiles of $Y^d_i$ is entirely analogous, and leads to the identifying expression
$$
...
In the next section, we our discussion of identification and consider identification under the (conditional) unconfoundedness assumption that $Y_i^x\independent X_i|W_i$.