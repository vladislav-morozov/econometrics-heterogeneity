--- 
description: "Learn how stationarity panel data can identify average marginal effects in nonparametric models with infinite unobserved heterogeneity (Lecture Notes)."
...
This section shows how panel data and stationarity assumptions can overcome heterogeneity bias to identify average marginal effects in model ([-@eq-nonparametric-model-full-nonsep]).

By the end of this section, you should be able to: 

- Explain how finite differences can be used to motivate the identification of average marginal effects.
...
This shows that we can observe the difference in outcomes directly in panel data for near-stayers — units whose covariates change only slightly over time [$\curl{X_{i1}=x, X_{i2}=x+h}$ for small $h>0$;  see e.g. @Sasaki2021]. It also determines the population for which we are able to identify the parameter of interest. We return to this topic below. 
...
\begin{aligned}
 & \E\left[ \partial_x\phi(x, a, u)|X_{i1}=X_{i2}=x \right] \\
 & = \lim_{h\to 0} \left[ \dfrac{Y_{i2}-Y_{i1}}{h}|X_{i1}=x, X_{i2} = x+h \right].
\end{aligned}
...
Before proceeding, we briefly discuss the population for which @eq-nonparametric-avg-me-identification-result identifies marginal effects.

Specifically, @eq-nonparametric-avg-me-identification-result identifies the average effect of an infinitesimal change in $x$ for a population known as *stayers at $x$* — units with identical values of $X_{i1}$ and $X_{i2}$. Stayers represent an important subpopulation in nonparametric panel data analysis, dating back to the seminal work of @Chamberlain1982.
