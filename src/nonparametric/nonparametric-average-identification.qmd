...
With identification in hand, we now analyze the behavior of this expression as $h\to 0$ to ensure that it converges to a marginal effect. Establishing convergence requires some continuity assumptions on the relationship between $X_{it}$, $A_i$ and $U_{it}$. To motivate their form, we write out ([-@eq-nonparametric-avg-key-exp]) as an integral, similarly to how we proceeded in the previous section. To that end, let 
...
To convert this representation into a true marginal effect, we analyze the integrand in @eq-nonparametric-avg-almost-me.. In expectation form, we can represent it as 
$$
