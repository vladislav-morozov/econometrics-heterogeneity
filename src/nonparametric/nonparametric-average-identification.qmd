...
# Nonparametric Identification of Average Effects {#sec-nonparametric-avg-id}

::: {.callout-note appearance="simple" icon=false}

## Summary and Learning Outcomes

This section shows how stationary panel data can overcome heterogeneity bias and identify average marginal effects even with infinite-dimensional unobserved heterogeneity and without randomized treatment assignment.
...
This shows that we can observe the difference in outcomes directly in panel data for near-stayers — units whose covariates change only slightly over time ($X_{i1}=x, X_{i2}=x+h$ for small $h>0$;  see @Sasaki2021 and the references cited therein). It also determines the population for which we are able to identify the parameter of interest. We return to this topic below. 
...
Observe that the latter two densities do not depend on $t$ by assumption of stationarity! This property is crucial to reducing ([-@eq-nonparametric-avg-key-exp]) to an integral of a finite difference in $\phi$.
...
\end{aligned}
$$
1. The conditional density of $U_{i2}$ is equal to the time-invariant density $f_{U_{it}|X_{i1}, X_{i2}, A_i}$: 
$$
\begin{aligned}
& = \int  \phi(x+h, a, u_2) f_{A_i|X_{i1}, X_{i2}}(a|x, x+h)
	\\
...
\end{aligned}
$$
1. A symmetric argument applies to the integral with $\phi(x, a, u_1)$.
...
In the next section, we refine @eq-nonparametric-avg-me-identification-result to provide an even more explicit identification result and discuss estimation. 
...
1. **Empirical relevance**: In many microdata settings, stayers and near-stayers comprise a substantial share or an outright majority of the population. See, for example, the evidence and discussion in @Sasaki2021.
