...
In the preceding sections, we focused on average marginal effects, which are basic summaries of how treatments influence outcomes. However, as emphasized in our discussion of linear models, higher-order features — variances, other moments, and distributions — are equally important.  
...
However, it turns out, the approach based on the second moment of  finite differences does succeed in a particular class of models where   $A_i$ remains unrestricted, but $U_{it}$ is assumed to enter additively. Specifically, we consider model ([-@eq-nonparametric-model-u-sep])
$$
	Y_{it}^x = \phi(x, A_i)  + U_{it}, \quad i=1, \dots, N, \quad t=1, 2.
$$ 
We drop the assumption of stationarity on $U_{it}$. We only assume that that the conditional mean of $U_{it}$ is invariant over time. Moreover, we assume that it is zero in the sense of mean exogeneity:
$$ 
\begin{equation}
	 \E[U_{it}|X_{i1}, X_{i2}, A_i] =0.
\end{equation} 
$$ 
We will further assume that $U_{i1}$ and $U_{i2}$ are uncorrelated conditional on $(X_{i1}, X_{i2})$. This model was considered by @Morozov2023jmp.

As above, the parameter of interest is the variance of marginal effects for stayers:
$$
	 \var(\partial_x \phi(x, \alpha_i)|x_{i1}=x_{i2}=x).
$$
By additive separability, the distribution of $U_{it}$ does not influence the marginal effects anymore.
