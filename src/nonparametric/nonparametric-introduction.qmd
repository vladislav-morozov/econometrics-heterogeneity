...
$$
	Y_i = \phi(\bX_i, A_i), \quad {}_{i=1,\dots, N},
...
$$
 Y_{it} = \phi(\bX_{it}, A_i, U_{it}), \quad {}_{i=1,\dots, N}^{t= 1, \dots, T},
...
To make progress, we focus on two general but tractable versions of the general nonparametric panel model ([-@eq-nonparametric-generic-panel]) in these notes. In both cases, we will assume that the outcome $Y_{it}$ is continuous and that $T=2$.
...
where for simplicity we assume that $X_{it}$ is scalar. The variable $X_{it}$ is assumed to be continuously distributed, and $\phi$  is continuous in $X_{it}$ for all values of $(A_i, U_{it})$.
