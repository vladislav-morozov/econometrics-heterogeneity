--- 
description: "Explore nonparametric models with unobserved heterogeneity. Identification of  minimal functional form assumptions. (Lecture Notes)"
...
- Write down generic nonparametric models with unobserved heterogeneity in cross-sectional and panel settings.
- Understand key identification challenges and the assumptions commonly used to address them.
...
To make progress, we focus on two general but tractable versions of the general nonparametric panel model ([-@eq-nonparametric-generic-panel]) in these notes. In both cases, we will consider models with continuous outcomes and assume that $T=2$. 
