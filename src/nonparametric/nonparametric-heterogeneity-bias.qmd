...
# Average Effects and Heterogeneity Bias {#sec-nonparametric-heterogeneity-bias}
...
Note that it is also possible to  consider the case where $(A_i, U_{it})$ and $X_{it}$ are independent only conditionally on some additional observed variable  $Z_{it}$ (ignorability/unconfoundedness assumption). The results are then conditional on $Z_{it}$. See @Altonji2005 for this flavor of results in continuous settings.
