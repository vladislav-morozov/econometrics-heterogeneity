...
With the machinery of local polynomial estimators, it is now easy to construct an estimator for the average marginal effect of stayers. We simply take use $Y_{i2}-Y_{i1}$ as the dependent variable in @eq-nonparametric-avg-lp and select the target point as $(x_1, x_2)=(x, x)$. The third coordinate of $\hat{\bbeta}(x, x)$ is an estimator for the partial derivative of $\E[Y_{i2}-Y_{i1}|X_{i1}=x_1, X_{i2}=x_2]$ with respect to $x_2$, evaluated at $(x, x)$ — precisely the average marginal effect of interest by  @eq-nonparametric-avg-est-me:
$$
...
Before move on to generalizations of the identification result, it is worth discussing @eq-nonparametric-avg-est-me somewhat more. The preceding identification and estimation arguments were based on the one-sided finite difference $(\phi(x+h, a, u)-\phi(x, a, u))$. However, it is possible to also work with central finite differences of the form [\phi(x+h, a, u)-\phi(x-h, a, u)]$. An advantage of a central difference is that it usually [provides a better approximation](https://en.wikipedia.org/wiki/Finite_difference#Relation_with_derivatives) for the derivatives. 
...
\begin{aligned}
    ...
    & \quad -\partial_{x_1}  \E[Y_{i2}-Y_{i1}|X_{i1}=x_1, X_{i2}=x_2]     \Bigg)\Big|_{(x_1, x_2)=(x, x)}.
\end{aligned}
$$
A symmetric estimator would then be based on the difference of third and the second coordinates of the local polynomial estimator $\hat{\bbeta}(x, x)$ — coordinates that correspond to the first two partial derivatives.
